...
public class NetworkStatusUtil {
    ...
    public static int NET_CONNECT_BAI_DU_OK = 1; //正常访问因特网状态 (可以访问百度)
    public static int NET_CONNECT_BAI_DU_TIMEOUT = 2; // 无法访问因特网状态 (不可以访问百度)
    public static int NET_NOT_PREPARE = 3; // 网络未准备好
    public static int NET_ERROR = 4; //网络 错误
                ...
                if (networkinfo != null) {
                    if (networkinfo.isAvailable() && networkinfo.isConnected()) {
                        if (!connectionNetwork())
                            return NET_CONNECT_BAI_DU_TIMEOUT;
                        else
                            return NET_CONNECT_BAI_DU_OK;
                    } else {
